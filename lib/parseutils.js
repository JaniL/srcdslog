...
module.exports.parseLineInfo = function(line,callback) {
  ...
  */
  ...
  var result = line.match(/^L \d\d\/\d\d\/\d\d\d\d - \d\d:\d\d:\d\d: "(.+)" killed "(.+)" with ["](.+)["] (\(headshot\))\n\u0000$/);
   if (result !== null) {
    module.exports.parsePlayer(result[1], function(player) {
  	var player1 = player;
		module.exports.parsePlayer(result[2], function(player) {
			var player2 = player;
			return callback({ type: 'kill', player: player1, killed: player2, weapon: result[3], headshot:  true});
		});
    });
  }
  ...
  var result = line.match(/^L (\d\d\/\d\d\/\d\d\d\d) - (\d\d:\d\d:\d\d:) "(.+)" killed "(.+)" with ["](.+)["]\n\u0000$/);
   if (result !== null) {
    module.exports.parsePlayer(result[3], function(player) {
  	var player1 = player;
		module.exports.parsePlayer(result[4], function(player) {
			var player2 = player;
			return callback({ time: result[2], type: 'kill', player: player1, killed: player2, weapon: result[5], headshot: false });
		});
    });
  }
  ...
  if (result !== null) {
    module.exports.parsePlayer(result[1], function(player) {
      return callback({ type: 'trigger', player: player , event: result[2] });  
    });
  }
