...
exports.parseLineInfoSync = parseLineInfoSync;
...
function attachMetadata(base, metadata, includeArgs) {
    const result = { ...base };
...
function parseLineInfoSync(line) {
    ...
    if (result) {
        if (result[2]) {
            ...
            return attachMetadata({ type: "worldtrigger", trigger: result[1] }, metadata, true);
        }
    ...
    if (result) {
        ...
        return attachMetadata({ type: "teamtrigger", team: result[1], trigger: result[2] }, metadata, true);
    }
    ...
    if (result) {
        const metadata = parseMetadata(result[3] ?? "");
        return attachMetadata({ type: "picked up", player: parsePlayer(result[1]), item: result[2] }, metadata, true);
    }
    ...
    if (result) {
        const metadata = parseMetadata(result[2]);
        return attachMetadata({ type: "position_report", player: parsePlayer(result[1]), text: parseArgs(result[2]) }, metadata, false);
    }
...
function parseLineInfo(line, callback) {
